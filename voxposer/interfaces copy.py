...
from voxposer.utils import get_clock_time, normalize_vector, pointat2quat, bcolors, Observation, VoxelIndexingWrapper, DynamicObservation, IterableDynamicObservation
import numpy as np
...
class LMP_interface():
  ...
  def __init__(self, env, lmp_config, controller_config, planner_config, env_name='handover'):
    self._env = env
  ...
  def detect(self, obj_name):
    ...
    if obj_name.lower() in EE_ALIAS:
      ...
      table_max_world = np.array([x_max, y_max, 0.92])
      table_min_world = np.array([x_min, y_min, 0.92])
  ...
  def execute(self, movable_obs_func, affordance_map=None, avoidance_map=None, rotation_map=None,
    ...
    if affordance_map is not None:
      ...
      for plan_iter in range(1):
        ...
        # movable_obs = movable_obs_func.obs_dict
